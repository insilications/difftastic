...
/// An identifier to denote a specific version of a text document. This information usually flows from the client to the
/// server.
#[derive(Debug, Eq, PartialEq, Clone, Deserialize, Serialize)]
...
/// An identifier which optionally denotes a specific version of a text document. This information usually flows from
...
impl<'de> serde::Deserialize<'de> for WatchKind {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        WatchKind::from_bits(i)
            .ok_or_else(|| D::Error::invalid_value(de::Unexpected::Unsigned(u64::from(i)), &"Unknown flag"))
    }
}
...
impl PublishDiagnosticsParams {
    pub fn new(uri: Uri, diagnostics: Vec<Diagnostic>, version: Option<i32>) -> PublishDiagnosticsParams {
...
mod tests {
    ...
    fn workspace_edit() {
        ...
        test_serialization(
            &WorkspaceEdit {
                changes: Some(vec![("file://test".parse().unwrap(), vec![])].into_iter().collect()),
                document_changes: None,
                ..Default::default()
            },
            r#"{"changes":{"file://test":[]}}"#,
