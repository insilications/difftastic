...
pub struct Config {
    ...
    pub diagnostics_excluded_files: Vec<Uri>,
...
impl Config {
    fn parse_rooted_file_paths(&mut self, v: Vec<String>) -> anyhow::Result<Vec<Uri>> {
        Ok(v.into_iter()
            .map(|path| Uri::from_file_path(self.root_path.join(path)).expect("Root path is absolute"))
            .collect())
    }

    fn parse_optional_command(&mut self, v: Option<Vec<String>>) -> anyhow::Result<Option<Vec<String>>> {
