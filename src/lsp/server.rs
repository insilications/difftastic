...
impl Server {
    ...
    fn on_did_change(&mut self, params: DidChangeTextDocumentParams) -> NotifyResult {
        ...
        //         };
        //         vfs.change_file_content(file, del_range, &change.text).ok()?;
        //         Some(())
        //     })();
        //     if ret.is_none() {
        //         tracing::error!("File is out of sync! Failed to apply change for {uri}: {change:?}");
    ...
    //                 #[cfg(unix)]
    //                 use rustix::fs::{OFlags, OpenOptionsExt, fcntl_getfl, fcntl_setfl};
    ...
    //         }
    //         Err(err) => {
    //             client.show_message_ext(MessageType::ERROR, format!("Failed to load flake workspace: {err:#}"));
    //             return;
    //         }
    //     };
    ...
    //         tracing::debug!("Missing flake inputs: {:?}", missing_paths().collect::<Vec<_>>());
    ...
    //                     Your LSP client doesn't support confirmation. You can enable autoArchive in lsp
    // configuration.\                     ",
    //                 );
    //                 false
    //             }
    ...
    //             if let Err(err) = ret {
    //                 client.show_message_ext(MessageType::ERROR, format_args!("Failed to archiving flake: {err:#}"));
    ...
    //             }
    //         };
    //         flake_info.input_flake_outputs.insert(input_name.clone(), output);
    ...
    //     let msg = (error_cnt != 0).then(|| format!("{error_cnt}/{input_cnt} input(s) failed to load"));
    //     progress.done(msg);
    // }
    ...
    //     let inputs = flake_lock::resolve_flake_locked_inputs(&config.nix_binary, lock_src.as_bytes())
    //         .await
    ...
    //     ) != (&config.diagnostics_excluded_files, &config.diagnostics_ignored);
    ...
    // fn on_update_diagnostics(&mut self, UpdateDiagnostics(version, diags): UpdateDiagnostics) -> NotifyResult {
